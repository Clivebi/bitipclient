...
internal  interface CryptBlock {
    fun encrypt(src: ByteArray,offset:Int): ByteArray
    fun decrypt(src:ByteArray,offset:Int):ByteArray
}


internal  class RC4CryptBlock constructor(key:ByteArray,writeIV:ByteArray,readIV:ByteArray):
    CryptBlock {
...
internal  class RSACryptBlock: CryptBlock {
    ...
    override fun encrypt(src: ByteArray,offset:Int): ByteArray {
        ...
        EncodeHelper.writeInt(
            ret,
            EncodeHelper.OFFSET_TOTAL_SIZE,
            buf.size
        )
...
internal  class AuthPacket constructor(username:ByteArray,token:ByteArray,method:Byte,secret:ByteArray) {
...
internal  class RAWPacket {
    ...
    constructor(buffer:ByteArray,crypt: CryptBlock) {
        ...
        this.base = crypt.decrypt(buffer,
            EncodeHelper.OFFSET_PROTOCOL
        )
        if (getPayloadSize() + EncodeHelper.RESERVED_SIZE > base.size) {
            return
        }
        hash.update(base,
            EncodeHelper.OFFSET_PROTOCOL,getPayloadSize()+5)
        val md5 = hash.digest()
    ...
    fun getPayload():ByteArray {
        ...
        return base.copyOfRange(
            EncodeHelper.OFFSET_PAYLOAD,
            EncodeHelper.OFFSET_PAYLOAD +size)
    }

    fun setPayload(data:ByteArray) {
        System.arraycopy(data,0,base,
            EncodeHelper.OFFSET_PAYLOAD,data.size)
        setPayloadSize(data.size)
    }

    fun setPayload(data:ByteArray,offset: Int,len: Int) {
        System.arraycopy(data,offset,base,
            EncodeHelper.OFFSET_PAYLOAD,len)
        setPayloadSize(len)
    }
    ...
    private fun getPayloadSize():Int {
        val size = EncodeHelper.readInt(
            base,
            EncodeHelper.OFFSET_PROTOCOL_SIZE
        )
        if (size < 5) {
            return 0
        }
...
internal  class ProtocolTCPClient(private val cryptBlock: CryptBlock, val socket: Socket)  {
    ...
    fun readFrame():ByteArray? {
        val rsp = EncodeHelper.readPacket(
    ...
    companion object{
                      ...
                      port:Int): ResultWithError<ProtocolTCPClient?>
        {
            ...
            try {
                val secret =
                    EncodeHelper.randBuffer(64)
                val cryptBlock = RC4CryptBlock(
                    secret.copyOfRange(
                        0,
                        32
                    ), secret.copyOfRange(48, 64), secret.copyOfRange(32, 48)
                )
                val socket = Socket()
                socket.connect(InetSocketAddress(host,port),1000*15)
                if (!socket.isConnected){
                    return ResultWithError(
                        null,
                        "socket connect server failed"
                    )
                }
                connected = true
                val auth = AuthPacket(
