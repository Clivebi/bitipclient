...
class LocalVpnService : VpnService() {
    class CommandParameter(val user:String,val token:String,val server:String,val port:Int,val packageName:String)
        ...
        if (intent?.getStringExtra("COMMAND") == "STOP") {
            ...
            mParameter =
                CommandParameter(
                    ...
                    intent.getStringExtra(KEY_APP_PACKAGE_NAME)!!
                )
    ...
    companion object{
        ...
        const val KEY_APP_PACKAGE_NAME = "start.package.name"

        fun startVPNService(context:Context, param:CommandParameter,id: String): ComponentName? {
            val intent = Intent(context, LocalVpnService::class.java)
            intent.putExtra("COMMAND", "START")
            intent.putExtra(LocalVpnService.KEY_USER_NAME,param.user)
            intent.putExtra(LocalVpnService.KEY_PASSWORD,param.token)
            intent.putExtra(LocalVpnService.KEY_SERVER,param.server)
            intent.putExtra(LocalVpnService.KEY_SERVER_PORT,param.port)
            intent.putExtra(LocalVpnService.KEY_START_ID,id)
            intent.putExtra(LocalVpnService.KEY_APP_PACKAGE_NAME,param.packageName)
